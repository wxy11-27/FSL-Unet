...
def main():
    ...
    if args.arch == 'SSFCNN':
      ...
      model = MSDCNN(args.scale_ratio,
                     args.n_select_bands,
                     args.n_bands)
    elif args.arch == 'SSRNET' or args.arch == 'SpatRNET' or args.arch == 'SpecRNET'or args.arch == 'FSL-Unet':
    ...
    ref = to_var(test_ref.float()).detach()
    lr = to_var(test_lr.float()).detach()
    hr = to_var(test_hr.float()).detach()

    begin_time = time()
    if args.arch == 'SSRNET':
        out, _, _, _, _ = model(lr, hr)
